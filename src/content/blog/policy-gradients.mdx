...
$$
P(S_t \in A\mid \text{history up to } s) = P(S_t \in A \mid S_s, A_s),\quad s<t.
$$
...
The big $\mathbb{E}$  represents the expected value $\mathbb{E}_{\tau\sim\pi }[R(\tau)]$ of a certain policy $\pi$. It’s useful for determining how effective the policy is performing. We can use it to formally solve for the optimal policy,
...
The real world is high dimensional and continuous, so you cannot brute-force sample $V(s)$ to find a global optimum without risking catastrophe like having a robot arm swing at you, or drone fly an object.

Instead we can borrow concepts from supervised learning with gradient descent to optimise a policy parametrised over $\theta \in \mathbb{R}^d$ directly from interaction, updating $\pi_\theta$ with gradients of the expected return estimated from rollouts. In short, policy gradients *learn by doing*.

We borrow gradient updates from supervised learning, but we do not minimise a loss $\theta \leftarrow \theta - \eta \nabla_{\theta}J(\theta)$. In policy gradients we maximise an objective,

$$
J(\theta) = \mathbb E_{ \tau\sim \pi_{\theta}}\left[R(\tau)\right],
$$

by computing gradient ascent,

$$
\theta \leftarrow \theta + \alpha \nabla_{\theta}J(\theta).
$$

Beyond gradient-based optimisation, little carries over from supervised learning; techniques like dropout and very deep networks often do not transfer cleanly to RL and can hurt stability.

Computing $\nabla_{\theta}J(\theta)$ isn’t straight-forward. which becomes obvious after expanding the expression,
...
The computational bottleneck in Eq. 5 is the trajectory integral  $\int_\tau P(\tau|\theta)R(\tau)\,\mathrm{d}\tau$. In continuous MDPs the trajectory space is enormous and uncountable, involves unknown dynamics $P(s_{t+1} | s_t, a_t)$ and the integral has no closed form. We can use the policy gradient theorem to simplify Eq. 2, removing the intractable terms,
