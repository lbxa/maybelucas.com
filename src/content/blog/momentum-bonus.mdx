...
Breaking large insurmountable tasks into smaller manageable subtasks enables hyper-productivity. I found that completing a high volume of small tasks results in a larger area under the productivity curve due to a ‘momentum bonus’ when compared to completing fewer monolithic tasks.
...
<ins>Case 1</ins>: You fully complete task $A$ and move onto $B$ (elastic)

Since $m_av_a + m_bv_b = m_av_a' + m_bv_b'$ your posterior velocities remain unchanged i.e. you haven’t hit the wall yet. At this point you’ll have the maximum momentum prior to starting task $B$. God speed.

<ins>Case 2</ins>: You don’t fully complete task $A$ and move onto $B$ (inelastic)

This case falls under the condition where $0\le e < 1$ which means your posterior velocity will decrease from losses to the environment hence $m_av_a + m_bv_b = (m_a+m_b)v$. By definition we know that $v \le v_a, v_b$. We can re-arrange to get
