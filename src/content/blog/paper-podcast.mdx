...
I’ve extracted the text from PDF files and piped it to OpenAI’s voice API. It was cheaper to convert Andrej Karpathy’s PhD thesis to a 3hr mp3 file for \$3.20 than to print it for 0.2c/side (~$19 for 90 pages).

A lot of the time there are a few pages that aren’t worth transforming into audio such as references,  table of contents, appendices—so they get trimmed away.
...
But now all we have is a 3-hour MP3 file. How do we consume this easily? I want the convenience of having this available wherever I go, whenever I have a moment to spare. I ended up automating the mp3 creation and uploading to a GCP bucket, then hosting a Podcast RSS feed from GitHub pages for free.

After two o1-mini prompts I had a full podcast setup. It’s that easy. I copy pasted the feed URL [lbxa.github.io/arxiv-to-mp3/rss.xml](https://lbxa.github.io/arxiv-to-mp3/rss.xml) into the free Overcast app on iOS and voilà. Podcast.

I’m excited to see if this unlocks a new dimension of knowledge dissemination. I’ll either be listening to my favourite house tracks or seminal papers in science and technology. What a dichotomy. The random narration voice selection should also keep the Podcast quite dynamic with 6 co-hosts to listen to—more than the usual Podcast.
